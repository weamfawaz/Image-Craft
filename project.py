...
# Remove labels from the datasets
train_image_ds = train_ds.map(lambda x, y: x)
val_image_ds = val_ds.map(lambda x, y: x)
...
history = mobile_net.fit( 
    train_image_ds,  # Use the datasets without labels
